...
class UserService:
    ...
    async def save_quiz_score(self, user_id: int, book_id: str, book_title: str, score: int, total_questions: int, quiz_results: Optional[List[Dict]] = None) -> Dict[str, Any]:
        """Save a quiz score and detailed results to the database"""
        try:
            results_json = json.dumps(quiz_results) if quiz_results else '{}'
            row = await self.db.fetchrow("""
                INSERT INTO quiz_scores (user_id, book_id, book_title, score, total_questions, quiz_results, created_at)
                VALUES ($1, $2, $3, $4, $5, $6::jsonb, NOW())
                RETURNING id, user_id, book_id, book_title, score, total_questions, quiz_results, created_at
            """, user_id, book_id, book_title, score, total_questions, results_json)
            ...
            try:
                ...
                await self.db.execute("""
                    INSERT INTO user_activity (user_id, activity_type, book_id, metadata, created_at)
                    VALUES ($1, $2, $3, $4::jsonb, NOW())
                """, user_id, 'quiz_completed', book_id, metadata)
            except Exception as activity_err:
                print(f"⚠️ Failed to log quiz activity: {activity_err}")
                
            res = dict(row)
            if res.get('quiz_results'):
                if isinstance(res['quiz_results'], str):
                    res['quiz_results'] = json.loads(res['quiz_results'])
            return res
        except Exception as e:
            print(f"❌ Failed to save quiz score: {e}")
            raise e

    async def get_user_quiz_history(self, user_id: int, limit: int = 10) -> List[Dict]:
        """Get a user's quiz history with detailed results"""
        try:
            rows = await self.db.fetch("""
                SELECT id, user_id, book_id, book_title, score, total_questions, quiz_results, created_at
                FROM quiz_scores
                WHERE user_id = $1
            ...
            print(f"❌ Failed to get quiz history: {e}")
            return []