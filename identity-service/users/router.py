...
async def save_quiz_score(
    ...
    """Save a user's quiz score with detailed results"""
    try:
        score = await service.save_quiz_score(
            user_id=current_user["id"],
...
@router.get("/quiz/history", response_model=list[QuizScoreResponse])
